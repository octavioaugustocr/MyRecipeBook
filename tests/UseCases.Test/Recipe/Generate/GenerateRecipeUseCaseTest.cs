using CommonTestUtilities.AI.GoogleAI;
    ...
    public class GenerateRecipeUseCaseTest
    {
        ...
        public async Task Error_Duplicated_Ingredients()
        {
            ...
            (await act.Should().ThrowAsync<ErrorOnValidationException>())
                .Where(e => e.ErrorMessages.Count == 1 && e.ErrorMessages.Contains(ResourceMessagesException.DUPLICATED_INGREDIENTS_IN_LIST));
        }

        private static GenerateRecipeUseCase CreateUseCase(GeneratedRecipeDto dto)
        {
            var generateRecipeAI = GenerateRecipeGoogleAIBuilder.Build(dto);
